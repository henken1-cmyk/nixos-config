...
export function PrometheusButton() {
  const [cpuTemp, setCpuTemp] = createState("\u2013")
  const [cpuLoad, setCpuLoad] = createState("\u2013")

  async function poll() {
    const temp = await queryInstant(
      'avg(node_hwmon_temp_celsius{chip=~".*coretemp.*"})'
    )
    if (temp) setCpuTemp(`${Math.round(parseFloat(temp))}\u00B0`)

    const load = await queryInstant(
      '100 - (avg(rate(node_cpu_seconds_total{mode="idle"}[1m])) * 100)'
    )
    if (load) setCpuLoad(`${Math.round(parseFloat(load))}%`)
  }
  ...
  return (
    <button
      cssClasses={["prometheus-btn", "bar-btn"]}
      onClicked={() => {
        const win = app.get_window("monitor-popup")
        if (win) win.visible = !win.visible
      }}
      tooltipText="System Monitor"
    >
      <box>
        <label cssClasses={["bar-icon"]} label="󰔏" />
        <label cssClasses={["prometheus-label"]} label={cpuTemp} />
        <label cssClasses={["prometheus-sep"]} label="|" />
        <label cssClasses={["prometheus-label"]} label={cpuLoad} />
      </box>
    </button>
  )
}

/* ── Full monitoring popup ──────────────────────────────────── */
